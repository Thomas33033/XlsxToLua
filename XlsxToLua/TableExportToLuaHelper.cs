...
public class TableExportToLuaHelper
{
    ...
    public static bool ExportTableToLua(TableInfo tableInfo, out string errorString)
    {
        ...
        {
            // 将主键列作为key生成
            content.Append(_GetLuaTableIndentation(1));
        ...
        content.AppendLine("}");
        content.AppendLine("");
        content.AppendLine("function newindex_Update = function()");
        content.AppendLine("{");
        content.AppendLine("\tprint(\"can't modiy config\")");
        content.AppendLine("}");
        content.AppendLine("local t_temp = { }");
        content.AppendLine("for k, v in pairs(list) do");
        content.AppendLine("\tlocal temp_v = { }");
        content.AppendLine("\tsetmetatable(temp_v)");
        content.AppendLine("\t{");
        content.AppendLine("\t\t__index = v,");
        content.AppendLine("\t\t__newindex = newindex_Update,");
        content.AppendLine("\t}");
        content.AppendLine("\ttemp[k] = temp_v");
        content.AppendLine("end");
        content.AppendLine("g_"+tableInfo.TableName + "=temp");
